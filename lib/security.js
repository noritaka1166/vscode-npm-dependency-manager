...
class SecurityService {
  ...
  async getPackageSecurity({ name, resolvedVersion, dependency, lockPackage, lockInfo }) {
    let vulnerabilities = dependency?.vulnerabilities ? dependency.vulnerabilities : [];
    let auditStatus = dependency?.auditStatus ? dependency.auditStatus : '';
    let auditError = dependency?.auditError ? dependency.auditError : '';
    let transitiveVulnerabilities = dependency?.transitiveVulnerabilities ? dependency.transitiveVulnerabilities : [];
    let osvVulnerabilities = dependency?.osvVulnerabilities ? dependency.osvVulnerabilities : [];
    let transitiveOsvVulnerabilities = dependency?.transitiveOsvVulnerabilities ? dependency.transitiveOsvVulnerabilities : [];

    if ((!auditStatus || (auditStatus === 'unknown' && !auditError && !vulnerabilities.length)) && resolvedVersion) {
    ...
    if (!transitiveOsvVulnerabilities.length && lockPackage && getLockPath(lockPackage) && lockInfo.exists) {
      const lockOsvResult = await this.getOsvVulnerabilitiesForTree(lockInfo, getLockPath(lockPackage));
    ...
    return {
      ...
      auditStatus,
      auditError,
      maxSeverity: dependency?.maxSeverity ? dependency.maxSeverity : getMaxSeverity([...vulnerabilities, ...osvVulnerabilities]),
