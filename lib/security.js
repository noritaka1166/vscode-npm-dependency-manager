...
const CISA_VULNRICHMENT_BASE_URL = 'https://raw.githubusercontent.com/cisagov/vulnrichment/develop';
...
class SecurityService {
  ...
  async getAuditAdvisories(packageVersions) {
    ...
    try {
      ...
      // npm audit is best-effort; retain dependency metadata when the endpoint is unavailable.
  ...
  async getOsvVulnerabilities(packages) {
    ...
    for (const chunk of chunkArray(missing, 100)) {
      ...
      try {
        ...
        reportOptionalSecurityFailure('OSV batch lookup failed', error);
      ...
      await Promise.all(chunk.map(async (packageInfo, index) => {
        const key = getPackageVersionKey(packageInfo.name, packageInfo.version);
        const resultEntry = data?.results?.[index];
        const vulns = Array.isArray(resultEntry?.vulns) ? resultEntry.vulns : [];
  ...
  async getOsvVulnerabilityDetails(ids) {
    ...
    await Promise.all(uniqueIds.map(async (id) => {
      ...
      try {
        ...
        reportOptionalSecurityFailure(`OSV detail lookup failed for ${id}`, error);
  ...
  async getThreatIntelForCves(cves) {
    ...
    const [kev, ssvc] = await Promise.all([
      this.getKevCatalog(),
      this.getSsvcDecisionPoints(uniqueCves)
    ]);
  ...
  async getEpssScores(cves) {
    ...
    for (const chunk of chunkArray(missing, 100)) {
      try {
        ...
        reportOptionalSecurityFailure('EPSS lookup failed', error);
        chunk.forEach((cve) => {
  ...
  async getKevCatalog() {
    ...
    try {
      ...
      reportOptionalSecurityFailure('CISA KEV lookup failed', error);
    }
  ...
  async getSsvcDecisionPoints(cves) {
    const result = new Map();
    const missing = [];
    (cves || []).forEach((cve) => {
      if (this.ssvcCache.has(cve)) {
        result.set(cve, this.ssvcCache.get(cve));
      } else {
        missing.push(cve);
      }
    });
    ...
    for (const chunk of chunkArray(missing, 10)) {
      await Promise.all(chunk.map(async (cve) => {
        const entry = await this.getSsvcDecisionPointsForCve(cve);
        this.ssvcCache.set(cve, entry);
        result.set(cve, entry);
      }));
    }
  ...
  async getSsvcDecisionPointsForCve(cve) {
    const url = getCisaVulnrichmentUrl(cve);
    if (!url) {
      return null;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`CISA Vulnrichment returned ${response.status}`);
      }
      return getSsvcFromCveRecord(await response.json(), cve);
    } catch (error) {
      reportOptionalSecurityFailure(`CISA SSVC lookup failed for ${cve}`, error);
      return null;
    }
  }
}
...
function applyUnknownSecurityState(dependency) {
  ...
  dependency.securitySignals = { cves: [], kev: [], epss: [], ssvc: [], maxEpss: null };
}

function normalizeDirectSecurityState(vulnerabilities, osvVulnerabilities) {
  const auditUnknown = vulnerabilities.length && vulnerabilities.every((advisory) => advisory.auditError);
  const auditError = auditUnknown ? vulnerabilities[0]?.title : '';
  const normalizedVulnerabilities = auditUnknown ? [] : vulnerabilities;

  return {
    vulnerabilities: normalizedVulnerabilities,
    osvVulnerabilities,
    auditStatus: getDirectAuditStatus(auditUnknown, normalizedVulnerabilities, osvVulnerabilities),
    auditError
  };
}

function getDirectAuditStatus(auditUnknown, vulnerabilities, osvVulnerabilities) {
  if (auditUnknown && !osvVulnerabilities.length) {
    return 'unknown';
  }
  return vulnerabilities.length || osvVulnerabilities.length ? 'vulnerable' : 'ok';
}
...
function getTransitiveVulnerabilities(lockInfo, dependency, auditResult) {
  return collectTransitiveAdvisories(lockInfo, dependency, auditResult,
    (child) => auditResult.get(child.name) || [],
    (child, advisory) => `${child.path}:${advisory.title}:${advisory.severity}:${advisory.vulnerableVersions}`,
    (advisory) => !advisory.auditError
  );
}

function getTransitiveOsvVulnerabilities(lockInfo, dependency, osvResult) {
  return collectTransitiveAdvisories(lockInfo, dependency, osvResult,
    (child) => osvResult.get(getPackageVersionKey(child.name, child.version)) || [],
    (child, advisory) => `${child.path}:${advisory.id || advisory.title}`
  );
}
...
function collectTransitiveAdvisories(lockInfo, dependency, source, getAdvisories, getAdvisoryKey, includeAdvisory = () => true) {
  if (!dependency.lockPath || !lockInfo?.paths || !source) {
    return [];
  }
  ...
  const visit = (packageInfo) => {
    ...
    Object.keys(dependencies).forEach((dependencyName) => {
      ...
      getAdvisories(child).forEach((advisory) => {
        const key = getAdvisoryKey(child, advisory);
        if (!includeAdvisory(advisory) || seenAdvisories.has(key)) {
          return;
        }
...
function getCisaVulnrichmentUrl(cve) {
  const match = /^CVE-(\d{4})-(\d+)$/i.exec(String(cve || ''));
  if (!match) {
    return '';
  }

  const [, year, sequence] = match;
  const bucket = sequence.length > 3 ? `${sequence.slice(0, -3)}xxx` : '0xxx';
  return `${CISA_VULNRICHMENT_BASE_URL}/${year}/${bucket}/${cve.toUpperCase()}.json`;
}

function getSsvcFromCveRecord(record, cve) {
  const containers = Array.isArray(record?.containers?.adp) ? record.containers.adp : [];
  for (const container of containers) {
    const metrics = Array.isArray(container.metrics) ? container.metrics : [];
    for (const metric of metrics) {
      const other = metric?.other;
      if (String(other?.type || '').toLowerCase() !== 'ssvc' || !other.content) {
        continue;
      }
      const options = Object.assign({}, ...(Array.isArray(other.content.options) ? other.content.options : []));
      return normalizeSsvcEntry({
        cve: other.content.id || cve,
        exploitation: getSsvcOption(options, 'Exploitation'),
        automatable: getSsvcOption(options, 'Automatable'),
        technicalImpact: getSsvcOption(options, 'Technical Impact'),
        role: other.content.role || '',
        version: other.content.version || '',
        timestamp: other.content.timestamp || ''
      });
    }
  }
  return null;
...
function getSsvcOption(options, name) {
  const key = Object.keys(options || {}).find((option) => option.toLowerCase() === name.toLowerCase());
  return key ? String(options[key] || '').toLowerCase() : '';
}

function normalizeSsvcEntry(entry) {
  return {
    cve: String(entry.cve || '').toUpperCase(),
...
function buildSecuritySignals(advisories, threatIntel) {
  ...
  cves.forEach((cve) => {
    ...
    const ssvcEntry = threatIntel?.ssvc ? threatIntel.ssvc.get(cve) : null;
    if (kevEntry) {
      kev.push(kevEntry);
    }
    if (epssEntry) {
...
function reportOptionalSecurityFailure(context, error) {
