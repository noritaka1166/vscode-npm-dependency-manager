...
function getLockPackages(lockInfo) {
  let source = [];
  if (lockInfo?.paths instanceof Map && lockInfo.paths.size) {
    source = [...lockInfo.paths.values()];
  } else if (lockInfo?.packages instanceof Map) {
    source = [...lockInfo.packages.values()];
  }
