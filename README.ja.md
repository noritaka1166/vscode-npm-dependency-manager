...
- npm registry に有用な README が公開されていない場合、GitHub README への fallback を試みます。
...
6. サイドバーでパッケージを展開すると transitive dependencies を確認できます。
...
- npm audit checks には resolved package version が必要です。`Vulnerabilities not checked` と表示される場合は、`package-lock.json` を追加または更新してください。
